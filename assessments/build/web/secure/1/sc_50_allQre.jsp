...
<%
        ...
        {
            // We are removing individuals or downlines from an assessment
            String assessmentId = request.getParameter("assessmentid");
            String[] users = request.getParameterValues("users");
            String downlineId = request.getParameter("downline");

            try
            {
                if (downlineId != null)
                {
                    if (!Assignment.removeAssessmentFromDownline(assessmentId, downlineId))
                    {
                        errOccurred = true;
                        msg = "Failed to remove the Assessment " + assessmentId + " from the Downline " + downlineId;
                    }
                }
                else if (users != null)
                {
                    ...
                    for (int i = 0; i < users.length; i++)
                    {
                        ...
                        if (!Assignment.removeAssessmentFromUser(assessmentId, userId))
                        {
                            errOccurred = true;
                            msg += "Failed to remove the assessment " + assessmentId + " from the user " + userId;
                        }
                    }
                }
            }
...
<% if (userLevel == 1)
        {%>
<div class='btnBar' style="width:500px">
    <%}
     else
     {%>
    <div class='btnBar'>
...
%>
        ...
        <% if (userLevel == 1)
                {%><div class='rButtons' style="width:240px"><%}
         else
         {%><div class='rButtons' style="width:120px"><%}%>
                ...
                <form action='index.jsp' method='post' style="float:right">
                    <input type='submit' class='btn1' value="Remove Participants" onclick="return setQREs(4);" />
                    <input type='hidden' name='qre'	id='Qre4'/>
                    <input type='hidden' name='qrenm' id='QreName4'/>
...
<input type="hidden" value="Page=50"/>